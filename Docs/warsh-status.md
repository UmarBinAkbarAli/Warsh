...
1. **`GOOGLE_PLAY_SERVICE_ACCOUNT_KEY` is malformed in production — every real purchase currently fails to activate.** Confirmed live on 2026-08-26: a throwaway account's session hit `POST /api/subscription/verify` on `https://api.warsh.app` with a bogus purchase token and got back `{"error":"Google Play service account key is not valid JSON.","code":"store_not_configured"}` (`warsh-backend/lib/storeVerification.ts` fails closed by design rather than granting unverified access — see `getGoogleAccessToken`). This is also what the in-app "Purchase needs attention" alert in `paywall.tsx` reports; it is not caused by a test card. Google completes the charge, but Warsh's server can't confirm it, so no subscription/Noor-pack purchase can currently be granted for any user on any device. Likely cause: the service-account JSON's `private_key` field had its escaped `\n` sequences turned into real line breaks when the value was set in Vercel, breaking JSON syntax. Fix: re-set `GOOGLE_PLAY_SERVICE_ACCOUNT_KEY` in Vercel production from the original Google Cloud service-account JSON, minified to one line (e.g. `jq -c .`) to avoid reformatting, redeploy, then re-run the same live diagnostic to confirm `store_not_configured` no longer occurs.
2. **Live IAP sandbox QA** — verify monthly and yearly subscription purchase, restore, acknowledgement, and Noor consumable behavior on a Play-installed build, after item 1 above is fixed. No real charge is required: add the test Gmail account under Play Console → Monetize setup → License testing, sign into that account on the test device, and purchases against the (now-production) listing are served as no-charge test transactions with accelerated renewal/expiry for testing that behavior too.
3. **Target-audience decision** — either select adults only for the simplest launch or implement the required age/minor handling before keeping ages 13–17.
4. **Latest-build device QA** — verify `VERB_PATTERN`, `AUDIO_RECOGNITION`, `WRITE_ARABIC`, and `HARAKAH_PLACEMENT` on a physical Android device.
...
6. **Confirm Play Console Payments profile (merchant account) is fully verified.** Owner reports a non-tester phone was also served a test-card-only checkout against the live production listing; the most likely cause is an incomplete Payments profile, which Google gates to test-purchase-only for all users regardless of License Testing settings, independent of item 1. Not independently verified — this session has no Play Console access.

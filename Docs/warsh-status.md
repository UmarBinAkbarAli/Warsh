...
1. **Live IAP sandbox QA** — remaining: the Noor consumable path and expiry/downgrade when a period lapses. Yearly purchase, restore/re-entitlement after reinstall, and acknowledgement were verified on device 2026-08-29 (below); monthly was verified earlier the same day. The server-side verification blocker that previously made this impossible (below) is now fixed.
   ...
   - **ROOT CAUSE of that third fault, still open: Real-Time Developer Notifications have never been delivered.** Play Console → Monetization setup has RTDN enabled against topic `projects/umar-tools-27994/topics/warsh-play-notifications`, but the Pub/Sub Subscriptions list for project `umar-tools-27994` is **empty** — the topic has no subscription at all, so nothing has ever been pushed to `/api/webhooks/google`. Every renewal, cancellation, refund and revocation notification since launch has been published and dropped. The lazy refresh above covers renewals and expiry; it does **not** give prompt handling of refunds/revocations, which RTDN's `SUBSCRIPTION_REVOKED` path exists to cut immediately. Fix: create a push subscription on that topic pointing at `https://api.warsh.app/api/webhooks/google?token=<GOOGLE_PLAY_NOTIFICATION_WEBHOOK_SECRET>` (or configure the OIDC push and set `GOOGLE_PLAY_PUBSUB_AUDIENCE` / `GOOGLE_PLAY_PUBSUB_SERVICE_ACCOUNT`, which the handler already prefers).

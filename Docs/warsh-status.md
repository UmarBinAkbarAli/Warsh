...
5. **RESOLVED 2026-08-27 — Google Play Payments profile is now set up.** Owner completed enrollment and Google confirmed the bank account by email. Independently reconfirmed live via Play Console → Settings → Payments profile on 2026-08-27: "How you get paid" now shows a verified bank account (PK•• •••• •••• •••• 0743, Umar Akbar) under "Manage payment methods." This closes out the 2026-08-26 finding below (kept for history): at that time the page showed no business/bank info, only a promotional "enroll" card, consistent with $0.00 lifetime revenue. Also reconfirmed live: License Testing is configured (an "Early Testers" email list, 42 addresses, active, `RESPOND_NORMALLY`), and the owner's own account (`umarakbar73456@gmail.com`) is on that list.
   - **New, separate, still-open issue found 2026-08-27:** a real purchase attempt from the app (screenshots: paywall shows a test card, "Continue Monthly" accepted, then in-app "Couldn't confirm subscription (invalid_purchase)", followed by a genuine Google Play "Start using your Warsh subscription" notification) was traced live in Vercel production logs (`warsh` project, `/api/subscription/verify`, `19:20:04` and `19:20:13` PKT). The actual Google error is **not** a generic per-token rejection — it's `Google subscriptionsv2 HTTP 404: {"error":{"code":404,"message":"No application was found for the given package name.", "reason":"applicationNotFound", ...}}`. This is a different failure than the 2026-08-26 finding (which returned a token-specific rejection, proving the package/app *did* resolve then). The app-level app-resolution step is now failing for the `warsh-play-verifier@warsh-production.iam.gserviceaccount.com` service account, even though Play Console → Users and permissions confirms that account still has Admin (13/13) permissions on the Warsh app (`com.warsh.app`). `GOOGLE_PLAY_PACKAGE_NAME` in Vercel is stored as a write-only Secret, so its exact current value could not be visually confirmed in this session. **Confirmed account/config-level, not a one-off**: repeated the 2026-08-26-style live check — registered a throwaway account, called `POST /api/subscription/verify` with a deliberately bogus token (`bogus-diagnostic-token-1234567890`) at `20:06:07` PKT, and the response was the identical `applicationNotFound` 404, not the old token-specific rejection. Since a bogus token now fails the same way a real one does, the app-resolution step is broken for every call from this service account right now, independent of which token is presented. The diagnostic account was deleted via `DELETE /api/users/me` immediately after. Not yet root-caused beyond that. Leading candidate: a transient Google-side API-access propagation delay triggered by today's Payments-profile enrollment (Google documents that Play Console permission/account changes can take time to propagate to the Android Publisher API) — worth simply retrying in a few hours before assuming a real break. If it is still failing after 2026-08-28, treat it as a real config problem and consider regenerating the service account key again (same remedy as the 2026-08-26 fix) or opening a Google Play support case, since Play Console's own UI shows nothing wrong (app is Production, service account has Admin/13-permission access to `com.warsh.app`).

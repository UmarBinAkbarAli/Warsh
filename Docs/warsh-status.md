...
5. **No Google Play Payments profile exists — every purchase on every device is forced into test-only mode until this is set up.** Confirmed live on 2026-08-26 via Play Console (Settings → Payments profile for developer account `6260301420448135151`): the page shows no business/individual info, no bank account, no verification status — only a promotional "enroll for the 15% service fee" card, meaning no profile has ever been created. This is consistent with the account's all-time **$0.00 total revenue**. Without a Payments profile, Google Play cannot process real money and silently serves every user, tester or not, a test-purchase-only checkout — this is what explains the non-tester phone seeing a test card. Cannot be completed by an agent: creating a Payments profile requires entering real bank account and tax/government-ID details. Owner action required at Play Console → Settings → Payments profile. Separately confirmed: License Testing is already configured (an "Early Testers" email list, 42 addresses, active, `RESPOND_NORMALLY`) — item 1's IAP QA can proceed as soon as this is done and the QA tester's account is on that list.
